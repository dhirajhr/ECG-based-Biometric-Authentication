function avgg = dwt_avg(ecg)
   % ecg=noiseremoval(ecg);
    
   % N=ecg(:,1);
    %X=ecg(:,2);
    %some changes for arduino
    X=ecg(:,1);
   % [Xpa,Xpb,D,F,Der]=lynfilt2(1,X,500,0);
    %ecg=[N F];
   % ecg=noiseremoval(ecg);
    %plot(N,X);
    ...
    [amp,index,delay,ecg_d]=pan_tompkin2(X,fs,0); %  amp - amplitude of Rpeaks and index f R peaks
    %ecg_d is derivative filtered signal. Use it if dataset has noisy
    %signals
